...
%        % Check  consistency of freqs
  ...
  % ---------------------------------
  defdowncom   = 'eeg_SASICA([],''eegplot(''''defdowncom'''',   gcbf);'');'; % push button: create/remove window
  defmotioncom = 'eeg_SASICA([],''eegplot(''''defmotioncom'''', gcbf);'');'; % motion button: move windows or display current position
  defupcom     = 'eeg_SASICA([],''eegplot(''''defupcom'''',     gcbf);'');';
  defctrldowncom = 'eeg_SASICA([],''eegplot(''''topoplot'''',   gcbf);'');'; % CTRL press and motion -> do nothing by default
...
% Five move buttons: << < text > >> 
  ...
  u(1) = uicontrol('Parent',figh, ...
	...
	'Callback',['global in_callback;', ...
                ...
                '    try eeg_SASICA([],''eegplot(''''drawp'''',1);'');', ...
  ...
  u(2) = uicontrol('Parent',figh, ...
	...
	'Callback',['global in_callback;', ...
                'if isempty(in_callback);in_callback=1;', ...
                '    try eeg_SASICA([],''eegplot(''''drawp'''',2);'');', ...
                '        clear global in_callback;', ...
                '    catch error_struct;', ...
                '        clear global in_callback;', ...
                '        throw(error_struct);', ...
  ...
  u(5) = uicontrol('Parent',figh, ...
	...
	'Callback', 'eeg_SASICA([],''eegplot(''''drawp'''',0);'');' );
  u(3) = uicontrol('Parent',figh, ...
	...
	'Callback',['global in_callback;', ...
                'if isempty(in_callback);in_callback=1;', ...
                '    try eeg_SASICA([],''eegplot(''''drawp'''',3);'');', ...
                '        clear global in_callback;', ...
                '    catch error_struct;', ...
                '        clear global in_callback;', ...
                '        throw(error_struct);', ...
  ...
  u(4) = uicontrol('Parent',figh, ...
	...
	'Callback',['global in_callback;', ...
                'if isempty(in_callback);in_callback=1;', ...
                '    try eeg_SASICA([],''eegplot(''''drawp'''',4);'');', ...
                '        clear global in_callback;', ...
                '    catch error_struct;', ...
                '        clear global in_callback;', ...
                '        error(error_struct);', ...
...
% Text edit fields: ESpacing
  ...
  u(6) = uicontrol('Parent',figh, ...
	...
	'Callback', 'eeg_SASICA([],''eegplot(''''draws'''',0);'');' );
...
% ESpacing buttons: + -
  u(7) = uicontrol('Parent',figh, ...
	...
	'string','+',...
	'FontSize',8,...
	'Callback','eeg_SASICA([],''eegplot(''''draws'''',1);'');');
  u(8) = uicontrol('Parent',figh, ...
	...
	'string','-',...
	'FontSize',8,...
	'Callback','eeg_SASICA([],''eegplot(''''draws'''',2);'');');
...
cb_envelope = ['g = get(gcbf,''userdata'');'...
    ...
    'eeg_SASICA([],''eegplot(''''drawp'''',0);'');clear g;'];
...
if ~isempty(g.data2)
  ...
  if ~isempty(g.events)
      u(17) = uicontrol('Parent',figh, ...
                        'Units', 'normalized', ...
                        'Position',posbut(17,:), ...
                        'string', 'Event types', 'callback', 'eeg_SASICA([],''eegplot(''''drawlegend'''',gcbf);'');');
  end;
  ...
  uimenu('Parent',m(7),'Label','Edit figure', 'Callback', 'eeg_SASICA([],''eegplot(''''noui'''');'');');
  ...
  uimenu('Parent',m(8),'Label','Print','tag','printcommand','callback',...
  		['RESULT = inputdlg( { ''Command:'' }, ''Print'', 1,  { ''print -r72'' });' ...
		 'if size( RESULT,1 ) ~= 0' ... 
		 '  eval ( RESULT{1} );' ...
		 'end;' ...
		 'clear RESULT;' ]);
  ...
  m(11) = uimenu('Parent',m(1),'Label','Data select/mark', 'tag', 'displaywin', ...
          ...
          uimenu('Parent',m(11),'Label','Hide marks','Callback', ...
            ...
            'set(gcbf, ''userdata'', g);' ...
            'eeg_SASICA([],''eegplot(''''drawb'''');''); clear g;'] )
  ...
  % Scale Eye %%%%%%%%%
  timestring = ['[OBJ1,FIG1] = gcbo;',...
	        'eeg_SASICA([],''eegplot(''''scaleeye'''',OBJ1,FIG1);'');',...
		'clear OBJ1 FIG1;'];
  m(7) = uimenu('Parent',m(1),'Label','Show scale','Callback',timestring);
  
  % Title %%%%%%%%%%%%
  uimenu('Parent',m(1),'Label','Title','Callback','eeg_SASICA([],''eegplot(''''title'''');'');')
  ...
  uimenu('Parent',m(2),'Label','Time range to display',...
      'Callback','eeg_SASICA([],''eegplot(''''window'''')'');')
  
  % Electrode window %%%%%%%%
  uimenu('Parent',m(2),'Label','Number of channels to display',...
      'Callback','eeg_SASICA([],''eegplot(''''winelec'''')'');')
  ...
  uimenu('Parent',m(6),'Label','Load .loc(s) file',...
      'Callback','eeg_SASICA([],''eegplot(''''loadelec'''')'');')
  ...
  complotevent = [ 'tmpg = get(gcbf, ''userdata'');' ...
                  'tmpg.plotevent = ''on'';' ...                  
                  'set(gcbf, ''userdata'', tmpg); clear tmpg; eeg_SASICA([],''eegplot(''''drawp'''',0)'');'];
  comnoevent   = [ 'tmpg = get(gcbf, ''userdata'');' ...
                  'tmpg.plotevent = ''off'';' ...                  
                  'set(gcbf, ''userdata'', tmpg); clear tmpg; eeg_SASICA([],''eegplot(''''drawp'''',0)'');'];
  comeventmaxstring   = [ 'tmpg = get(gcbf, ''userdata'');' ...
                'tmpg.plotevent = ''on'';' ...
                'set(gcbf, ''userdata'', tmpg); clear tmpg; eeg_SASICA([],''eegplot(''''emaxstring'''');'');']; % JavierLC      
  comeventleg  = [ 'eeg_SASICA([],''eegplot(''''drawlegend'''',gcbf)'');'];
...
%             ylims=ylim;
  ...
  case 'draws'
    ...
    if p1 == 1
      	g.spacing= g.spacing*1.5; % increase g.spacing(5%)
	elseif p1 == 2
		g.spacing= max(0,g.spacing/1.5); % decrease g.spacing(5%)
    end
...
%     end;
  ...
  case 'window'  % change window size
    ...
    % -------------------------------------
    g = get(gcf,'UserData');
	result       = inputdlg( { fastif(g.trialstag==-1,'New window length (s):', 'Number of epoch(s):') }, 'Change window length', 1,  { num2str(g.winlength) });
...
{ 'Number of channels to display:' } , 'Change number of channels to display', 1,  { num2str(g.dispchans) });
   ...
   case 'emaxstring'  % change events' string length  ;  JavierLC
      ...
      % -------------------------------------
      g = get(gcf,'UserData');
      result = inputdlg({ 'Max events'' string length:' } , 'Change events'' string length to display', 1,  { num2str(g.maxeventstring) });
      if size(result,1) == 0 return; end;                  
  ...
  case 'title'
	...
	if ~isempty(h)
		result       = inputdlg( { 'New title:' }, 'Change title', 1,  { get(h(1), 'string') });
		if ~isempty(result), set(h, 'string', result{1}); end;
	else 
		result       = inputdlg( { 'New title:' }, 'Change title', 1,  { '' });
  ...
  case 'noui'
   ...
   case 'zoom' % if zoom
      ...
      if exist('p2', 'var') == 1
          if verLessThan('matlab','8.4.0')
              set(gcbf, 'windowbuttondownfcn', [ 'zoom(gcbf,''down''); eeg_SASICA([],''eegplot(''''zoom'''',gcbf,1)'');' ]);
          else
              % This is failing for us: http://undocumentedmatlab.com/blog/enabling-user-callbacks-during-zoom-pan
              ...
              set(gcbf, 'WindowButtonDownFcn', [ 'zoom(gcbf); eeg_SASICA([],''eegplot(''''zoom'''',gcbf,1)'');' ]);
              warning(wtemp);
          end
      end;
